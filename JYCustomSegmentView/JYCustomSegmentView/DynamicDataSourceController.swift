...
class DynamicDataSourceController: UIViewController {
    ...
    private var dataArray:[String] = ["舒适","策士","基本数据","基本类型"] {
        didSet {
...
//        self.setHeaderViewOptions()
        let testView = JYTestView()
        testView.btn1ActionBlock = {[weak self] in
            self?.dataArray = ["舒适","策士基本数据"]
        }
        contentView.addViewsToContentView(subViews: [testView,JYTestView(),JYTestView(),JYTestView()])
